import 'package:class_room_chin/utils/debounce.dart';
...
class PresentWidget extends StatefulWidget {
  const PresentWidget(
      ...
      this.dismissDragLeftDrawer = true,
      this.dismissDragRightDrawer = true,
...
class _PresentWidgetAnimationState extends State<PresentWidget>
  ...
  bool get _isShowKeyboard => MediaQuery.of(context).viewInsets.bottom != 0;
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if (widget.controller.isShowingPresent ||
...
extension on _PresentWidgetAnimationState {
  ...
  void _handleChildHorizontalUpdate(DragUpdateDetails updateDetails) {
    if ((widget.dismissDragLeftDrawer || _isShowKeyboard) &&
        updateDetails.delta.dx > 0 &&
        _drawerController.value >= 0) return;
    if ((widget.dismissDragRightDrawer || _isShowKeyboard) &&
        updateDetails.delta.dx < 0 &&
        _drawerController.value <= 0) return;
    var delta = updateDetails.primaryDelta!;
  ...
  void _showPresent() {
    widget.controller.isShowingPresent = true;
    widget.controller.notifyListeners();
  }

  void _hidePresent() {
    widget.controller.isShowingPresent = false;
    widget.controller.notifyListeners();
  }
